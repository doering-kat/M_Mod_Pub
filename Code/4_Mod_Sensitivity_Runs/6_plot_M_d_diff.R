...
# Still need to create this, want one for M only with NOAA codes and regions already
# marked. 
mod_sum <- read.csv("./Derived_Data/4_Mod_Sensitivity_Runs/2_get_mod_summaries/all_mod_summary.csv") # originals

# add models that need to be redone. (because of issues with effective sample size)
mod_rerun_path <- "./Derived_Data/4_Mod_Sensitivity_Runs/4_rerun_mods"
# names of the models that were rerun in a vector
mod_rerun_names <- read.csv("./Derived_Data/4_Mod_Sensitivity_Runs/3_check_convergence/mods_to_rerun.csv")
mod_rerun_names <- mod_rerun_names$x

# create lists to store the model summaries
mods_rerun_sum <- vector("list", length(mod_rerun_names))
# load the mods and summary into the lists
for (i in seq_along(mod_rerun_names)){
  mods_rerun_sum[[i]] <- read.csv(paste0(mod_rerun_path, "/", mod_rerun_names[i], "_mod_summary.csv"))
}
...
der_dat_spec_path <- paste0(der_dat_gen_path, "/6_plot_M_d_diff")
# for figures
...
# rename the X column in the rerun df to param (to match with final_mod_sum)
mods_rerun_sum_df <- mods_rerun_sum_df %>% 
                      rename(param = X)
# bind the new model results with the other models
final_mod_sum <- bind_rows(final_mod_sum, mods_rerun_sum_df)
