...
#' @param M a dataframe with columns Year, M_annual, and NOAACODE
#' @param Yr a single integer value with the year of M_annual to plot.
#' @param NOAA an sp spatial polygon data.frame with the NOAACODE polygons.
#'  lab, a single logical value. True if NOAA code labels are desired on the plot,
#'  or False if no labels are wanteed.
#' @param NOAAlab an sp spatial points data.frame including the labels and 
#'  where they should be placed.
#' @param axes a signle logical value, true if want axes in northing and 
#'  easting, false if not.
#' @param axes_lab a single logical value, true if want x and y axis labels, 
#'  false if not.
#' @param title a single logical value, true if want the year as the title on 
#'  the plot.
#' @param title_text optional text to add to the title if title = FALSE.
#' @param write_file should the figure be written to a file? defaults to FALSE.
#' @param filepath specify where the plots should be made. Does not include the
#'  filename.
#' @param filename an optional name for  the file. should have extension ".pdf"
#' @param palette a character vector of color names. length should be 1 less
#'  than palette breaks.
#' @param palette_breaks a numeric vector specifying breaks in the scalebar. 
#'  This can be specified or left as optional. If not specified, function will
#'  calculate by scaling equally using the number of categories in the palette 
#'  between 0 and 1.
#' @return an sp plot object invisibly. Side effect is creates and saves plot 
#'  in a graphic device.)
Plot_M_Map_latlon <- function(M, colname = "M_Annual" , NOAA, lab = F, NOAAlab = NULL, 
  axes = F, axes_lab = F, title = T, title_text = "Year here", write_file = FALSE,
...
for (y in 1991:2017) {
  ...
  maps[[cnt]] <-   Plot_M_Map_latlon(M = tmp_M, 
                                     NOAA = NOAA_latlon, 
                                     colname = "M_annual", 
                                     filepath = fig_spec_path,
                                     axes = T,
                                     axes_lab = T,
                                     filename = paste0("M_", y, ".pdf"), 
                                     title_text = as.character(y))
  
  pdf(file.path(fig_spec_path, paste0("M_", y, ".pdf")), width = 9, height = 11)
  print(maps[[cnt]] + 
    latticeExtra::layer_(sp.polygons(NOAA_latlon,fill='black')))
  dev.off()
  cnt <- cnt + 1
}
...
Plot_M_Map_latlon(M = Avg_SD_M_df, NOAA = NOAA_latlon, colname = "Mean", filepath = fig_spec_path, axes = T,axes_lab = T, write_file = TRUE,
...
Plot_M_Map_latlon(M = Avg_SD_M_df, NOAA = NOAA_latlon, colname = "SD", filepath = fig_spec_path, axes = T,axes_lab = T, write_file = TRUE, 
